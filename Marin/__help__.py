...
for module_names in admin_mod_name:
    ...
    if admin_imported_module.__mod_name__.lower() not in ADMIN_IMPORTED:
        ADMIN_IMPORTED[
            admin_imported_module.__mod_name__.lower()
        ] = admin_imported_module
    else:
...
path = r"Marin/Plugins/Sudoers/"
sudoers_list_of_files = []
for root, dirs, files in os.walk(path):
    for file in files:
        sudoers_list_of_files.append(os.path.join(root, file))

sudoers_mod_name = [
    name[:-3].replace("/", ".").replace("\\", ".")
    for name in sudoers_list_of_files
    if isfile(name) and name.endswith(".py") and not name.endswith("__init__.py")
]

for module_names in sudoers_mod_name:
    sudoers_imported_module = importlib.import_module(module_names)
    if not hasattr(sudoers_imported_module, "__mod_name__"):
        sudoers_imported_module.__mod_name__ = sudoers_imported_module.__name__

    if sudoers_imported_module.__mod_name__.lower() not in SUDOERS_IMPORTED:
        SUDOERS_IMPORTED[
            sudoers_imported_module.__mod_name__.lower()
        ] = sudoers_imported_module
    else:
        raise Exception("Can't have two modules with the same name! Please change one")

    if (
        hasattr(sudoers_imported_module, "__help__")
        and sudoers_imported_module.__help__
    ):
        SUDOERS[sudoers_imported_module.__mod_name__.lower()] = sudoers_imported_module


path = r"Marin/Plugins/Extras/"
extras_list_of_files = []
for root, dirs, files in os.walk(path):
    for file in files:
        extras_list_of_files.append(os.path.join(root, file))

extras_mod_name = [
    name[:-3].replace("/", ".").replace("\\", ".")
    for name in extras_list_of_files
    if isfile(name) and name.endswith(".py") and not name.endswith("__init__.py")
]

for module_names in extras_mod_name:
    extras_imported_module = importlib.import_module(module_names)
    if not hasattr(extras_imported_module, "__mod_name__"):
        extras_imported_module.__mod_name__ = extras_imported_module.__name__

    if extras_imported_module.__mod_name__.lower() not in EXTRAS_IMPORTED:
        EXTRAS_IMPORTED[
            extras_imported_module.__mod_name__.lower()
        ] = extras_imported_module
    else:
        raise Exception("Can't have two modules with the same name! Please change one")

    if hasattr(extras_imported_module, "__help__") and extras_imported_module.__help__:
        EXTRAS[extras_imported_module.__mod_name__.lower()] = extras_imported_module


path = r"Marin/Plugins/User/"
user_list_of_files = []
for root, dirs, files in os.walk(path):
    for file in files:
...
for t_module_name in tools_mod_name:
    ...
    if tools_imported_module.__mod_name__.lower() not in TOOLS_IMPORTED:
        TOOLS_IMPORTED[
            tools_imported_module.__mod_name__.lower()
        ] = tools_imported_module
    else:
        raise Exception("Can't have two modules with the same name! Please change one")
...
def send_sudoers_help(chat_id, text, keyboard=None):
    if not keyboard:
        keyboard = InlineKeyboardMarkup(paginate_modules(0, SUDOERS, "sudoers"))
    dispatcher.bot.send_message(
        chat_id=chat_id,
        text=text,
        parse_mode=ParseMode.MARKDOWN,
...
def send_extras_help(chat_id, text, keyboard=None):
    if not keyboard:
        keyboard = InlineKeyboardMarkup(paginate_modules(0, EXTRAS, "extras"))
    dispatcher.bot.send_message(
        chat_id=chat_id,
        text=text,
        parse_mode=ParseMode.MARKDOWN,
...
def sudoers_help_button(update, context):
    query = update.callback_query
    mod_match = re.match(r"sudoers_module\((.+?)\)", query.data)
    prev_match = re.match(r"sudoers_prev\((.+?)\)", query.data)
    next_match = re.match(r"sudoers_next\((.+?)\)", query.data)
    back_match = re.match(r"sudoers_back", query.data)
...
def extras_help_button(update, context):
    query = update.callback_query
    mod_match = re.match(r"extras_module\((.+?)\)", query.data)
    prev_match = re.match(r"extras_prev\((.+?)\)", query.data)
    next_match = re.match(r"extras_next\((.+?)\)", query.data)
    back_match = re.match(r"extras_back", query.data)
    ...
    try:
        if mod_match:
            ...
            query.message.edit_caption(
                ...
                disable_web_page_preview=True,
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton(text="Back", callback_data="extras_back")]]
                ),
            )
    ...
    try:
        if mod_match:
            ...
            query.message.edit_text(
                text=HELP_STRINGS,
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup(paginate_modules(0, TOOLS, "tools")),
            )
