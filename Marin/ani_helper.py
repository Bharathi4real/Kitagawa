...
from Marin import OWNER, DOWN_PATH, anibot, LOG_CHANNEL_ID, has_user
        ...
        if func.__name__ not in [
            ...
            try:
                ...
                await message.reply_text(
                    "Click the below button to get results",
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton(
                                    "Click Here", callback_data=f"confirm_{key}"
                                )
                            ]
                        ]
                    ),
...
async def take_screen_shot(
    ...
    command = (
        f'''ffmpeg -ss {duration} -i "{video_file}" -vframes 1 "{thumb_image_path}"'''
    )
