...
def yone_bhelp_callback(update: Update, context: CallbackContext):
    first_name = update.effective_user.first_name
    query = update.callback_query
...
def main():
    ...
    sudoers_help_callback_handler = CallbackQueryHandler(
        sudoers_help_button, pattern=r"sudoers_.*", run_async=True
    )
    ...
    extras_help_callback_handler = CallbackQueryHandler(
        extras_help_button, pattern=r"extras_.*", run_async=True
    )

    about_callback_handler = CallbackQueryHandler(
        yone_about_callback, pattern=r"yone_", run_async=True
    )

    bhelp_callback_handler = CallbackQueryHandler(
        yone_bhelp_callback, pattern=r"bhelp_", run_async=True
    )
    ...
    dispatcher.add_handler(bhelp_callback_handler)
    dispatcher.add_handler(settings_handler)
    dispatcher.add_handler(help_callback_handler)
    dispatcher.add_handler(admin_help_callback_handler)
    dispatcher.add_handler(sudoers_help_callback_handler)
    dispatcher.add_handler(user_help_callback_handler)
    dispatcher.add_handler(tools_help_callback_handler)
    dispatcher.add_handler(settings_callback_handler)
    dispatcher.add_handler(migrate_handler)
    dispatcher.add_handler(extras_help_callback_handler)

    if WEBHOOK:
        LOGGER.info("Using webhooks.")
        updater.start_webhook(listen="1.1.1.1", port=PORT, url_path=TOKEN)
