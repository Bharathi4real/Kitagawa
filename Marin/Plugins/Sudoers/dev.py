...
from Marin import DEV_USERS, OWNER_ID, dispatcher, telethn, updater
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not Marin.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        Marin.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Marin.ALLOW_CHATS = False
    else:
