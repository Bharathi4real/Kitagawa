...
from Marin.utils.helper import (
    ANON_JSON,
    check_user,
    get_btns,
    AUTH_USERS,
...
from Marin.utils.db import get_collection
...
async def anime_cmd(client: Client, message: Message, mdata: dict):
    ...
    result = await get_anime(
        vars_, user=user, auth=auth, cid=gid if gid != user else None
    )
    if len(result) != 1:
...
async def manga_cmd(client: Client, message: Message, mdata: dict):
    ...
    result = await get_manga(
        qdb, 1, auth=auth, user=user, cid=gid if gid != user else None
    )
    if len(result) == 1:
        k = await message.reply_text(result[0])
        await asyncio.sleep(5)
...
@anibot.on_message(
    filters.command(
        ["flex", f"flex{BOT_NAME}", "user", f"user{BOT_NAME}"], prefixes=trg
    )
)
@control_user
...
@anibot.on_message(
    ~filters.private
    & filters.command(["settings", f"settings{BOT_NAME}"], prefixes=trg)
)
@control_user
...
@anibot.on_message(
    filters.command(
        ["me", f"me{BOT_NAME}", "activity", f"activity{BOT_NAME}"], prefixes=trg
    )
)
@control_user
...
async def favourites_cmd(client: Client, message: Message, mdata: dict):
    ...
    btn = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "ANIME", callback_data=f"myfavqry_ANIME_{query}_1_no_{user}"
                ),
                InlineKeyboardButton(
                    "CHARACTER", callback_data=f"myfavqry_CHAR_{query}_1_no_{user}"
                ),
                InlineKeyboardButton(
                    "MANGA", callback_data=f"myfavqry_MANGA_{query}_1_no_{user}"
                ),
            ],
            [InlineKeyboardButton("Profile", url=f"https://anilist.co/user/{query}")],
        ]
...
async def list_tags_genres_cmd(client, message: Message, mdata: dict):
    ...
    if (
        find_gc is not None
        and "gettags" in text.split()[0]
        and "gettags" in find_gc["cmd_list"].split()
    ):
        return
    if (
        find_gc is not None
        and "getgenres" in text.split()[0]
        and "getgenres" in find_gc["cmd_list"].split()
    ):
        return
...
async def page_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if media in ["ANIME", "MANGA"]:
        ...
        result = await (get_character if media == "CHARACTER" else get_airing)(
            query, int(page), auth=authbool, user=int(user)
        )
    pic, msg = result[0], result[1][0]
    if media == "AIRING":
    ...
    if (
        ...
        await cq.edit_message_media(
            InputMediaPhoto(
                no_pic[random.randint(0, 4)],
                caption="This material is marked 18+ and not allowed in this group",
            ),
            reply_markup=button,
        )
        return
...
async def anime_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    result = await get_anime(
        vars_, auth=authbool, user=user, cid=cid if cid != user else None
    )
...
async def toggle_favourites_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    result = (
        (
            await get_anime(
                {"id": idm}, auth=True, user=user, cid=gid if gid != user else None
            )
        )
...
async def update_anilist_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    rslt = await update_anilist(
        id_=idm, req=query[0], status=query[1], user=user, eid=eid
    )
    if rslt == "ok":
        await cq.answer("Updated")
    else:
        ...
        if "~!" in result and "!~" in result:
            ...
            button.append(
                [
                    InlineKeyboardButton(
                        text="View spoiler",
                        url=f"https://t.me/{bot}/?start=des_{ctgry}_{query}",
                    )
                ]
            )
    except TypeError:
        ...
        if spoiler is False:
            ...
            button.append(
                [
                    InlineKeyboardButton(
                        text="More Info",
                        url=f"https://t.me/{bot}/?start=des_{ctgry}_{query}_{kek}",
                    )
                ]
            )
    add_ = ""
            ...
            if page == "1":
                button.append(
                    [
                        InlineKeyboardButton(
                            text="Next",
                            callback_data=f"{kek}_{query}_{ctgry}{lsqry}{lspg}_{q[5] if len(q) != 5 else q[3]}_{int(page)+1}_{user}",
                        )
                    ]
                )
...
async def featured_in_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if total > 15:
        button.append(
            [
                InlineKeyboardButton(
                    text="Next", callback_data=f"lsca_{idm}_1_{qry}_{pg}_{auth}_{user}"
                )
            ]
        )
    if req is not None:
        button.append(
            [
                InlineKeyboardButton(
                    text="Manga", callback_data=f"lscm_{idm}_0_{qry}_{pg}_{auth}_{user}"
                )
            ]
        )
    button.append(
        ...
        if int(reqpg) == 0:
            button.append(
                [
                    InlineKeyboardButton(
                        text="Next",
                        callback_data=f"{req}_{idm}_{int(reqpg)+1}_{qry}_{pg}_{auth}_{user}",
                    )
                ]
            )
...
async def change_ui_btn(client: Client, cq: CallbackQuery):
    ...
    btn.append(
        [
            ...
            InlineKeyboardButton("UPPER", callback_data=f"cui_UPPER_{gid}"),
        ]
    )
