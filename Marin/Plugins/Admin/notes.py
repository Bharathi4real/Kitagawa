...
from Marin.Handlers.misc import build_keyboard, revert_buttons
from Marin.Handlers.msg_types import get_note_type
from Marin.Handlers.string_handling import (
...
def get(
    ...
    note_chat_id = privnote_chat_id or update.effective_chat.id
...
def make_note_url(chat_id, notename):
    ...
    return f"http://t.me/{dispatcher.bot.username}?start=privnote_{note_id}"
...
def list_notes(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    if private_notes := sql.get_privnotes_setting(chat_id):
        update.effective_message.reply_text(
...
GET_NOTES_DEEPLINK = CommandHandler(
    "start", get_notes_deeplink, filters=Filters.regex("privnote_"), run_async=True
)
LIST_NOTES_DEEPLINK = CommandHandler(
    "start", list_notes_deeplink, filters=Filters.regex("listnotes_"), run_async=True
)
